...
fn str() {
    ...
    match ctx.execute() {
        Ok(_) => unsafe {
            ...
            let x1_8_value = x1.wrapping_add(8).read_u32();
            assert_eq!(x1_8_value, 10000);
            // x1 + 16 should point to x1 (sp at the time)
            let x1_16_value_ptr = x1.wrapping_add(16).read_ptr();
            ...
            let x1_x2_shl_3 = x1.wrapping_add(x2_value.wrapping_shl(3) as usize).read_u32();
